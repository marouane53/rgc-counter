...
def annotate_points_interactive(image: np.ndarray, existing_points_yx: np.ndarray) -> np.ndarray:
    import napari
    ...
    with napari.gui_qt():
        ...
        def _save_and_close(v):
            holder["points"] = np.asarray(points_layer.data, dtype=float)
            v.close()
        ...
        print("Press 's' to save and close, or 'q' to close without saving.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for record in records:
        ...
        points = annotate_points_interactive(crop, existing_points)
